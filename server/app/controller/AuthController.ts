...
export const registerUser = async (
  ...
  try {
    // 1. Extract `username`, `email`, and `password` fields from `req.body`.
    const { username, email, password } = req.body;

    // 2. Validate that none of the three required fields are missing or empty (return 400 Bad Request if validation fails).
    ...
    // 3. Verify if the username or email is already taken using `findUserbyName` and `findUserbyEmail` (return 409 Conflict if a duplicate is found).
    ...
    // 4. Hash the raw password asynchronously using `hashPassword(password)`.
    const passwordHash = await hashPassword(password);

    // 5. Persist the new user credentials by calling `createUser({ username, email, passwordHash })`.
    const newUser = createuser({ username, email, passwordHash });

    // 6. Return the newly created user object, omitting the `passwordHash` field, alongside a 201 Created status.
...
export const loginUser = async (
  ...
  try {
    ...
    // 2. Validate that both fields are present and not empty strings (return 400 Bad Request if missing).
    if (!email || !password) {
      return res.status(400).json({ error: "Email and password are required" });
    }

    // 3. Look up the user record by username using `findUserByName` or using `findUserbyEmail` (return 401 Unauthorized if the user does not exist).
    const user = findUserbyEmail(email);
    if (!user) {
      return res.status(401).json({ error: "Invalid credentials" });
    }

    // 4. Compare the provided password with the stored hash using `comparePassword(password, user.passwordHash)` (return 401 Unauthorized if verification fails).
    const isMatch = await comparePassword(password, user.passwordHash);
    if (!isMatch) {
      return res.status(401).json({ error: "Invalid credentials" });
    }

    // 5. Transition the user's presence state to online by executing `updateUserStatus(username, "online")`.
    ...
    // 6. Populate the session store `req.session` with identifying data including `userId` and `username`.
    // @ts-ignore
    req.session.userId = user.id;
    // @ts-ignore
    req.session.username = user.username;

    // 7. Dispatch a success response in JSON format with a 200 OK status.
...
export const logoutUser = async (
  ...
  try {
    // 1. Retrieve the `username` saved inside the active session store.
    // @ts-ignore
    const username = req.session?.username;

    // 2. If the username is present, change their memory presence status to offline using `updateUserStatus(username, "offline")`.
    if (username) {
    ...
    // 3. Destroy the active session state by setting `req.session = null`.
    // @ts-ignore
    req.session = null;

    // 4. Send back a success JSON message indicating the logout process was completed with a 200 OK status.
